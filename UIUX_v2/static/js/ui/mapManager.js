...
(function(window) {
    ...
    const createMapManager = (options = {}) => {
        ...
        const addTrajectoryTimeMarkers = (ship, trajectoryEntries, nowCoords = null) => {
            if (!trajectoryEntries.length) {
                // If we have nowCoords (from track line end), at least show that
                if (nowCoords) {
                     const nowMarker = createNowMarker(nowCoords);
            ...
            // Use provided nowCoords (visual line end) or fallback to latestEntry
            const nowPos = nowCoords || latestEntry;
            const nowMarker = createNowMarker(nowPos);
            trajectoryTimeMarkers.push(nowMarker);
            ...
            TRAJECTORY_TIME_INTERVALS.forEach(interval => {
                ...
                // Ensure we don't draw a history marker on top of the Now marker
                if (closest && areCoordsDistinct(closest, nowPos)) {
        ...
        const drawTrajectory = (points, ship = null) => {
            ...
            if (ship && typeof getTrajectoryEntries === "function") {
                ...
                // Pass the last point of the visual line as the forced "Now" coordinate
                const visualEndPoint = points[points.length - 1];
                const nowCoords = { lat: visualEndPoint[0], lng: visualEndPoint[1] };
                addTrajectoryTimeMarkers(ship, trajectoryEntries, nowCoords);
            }
        ...
        const updateMapForShip = (card) => {
            ...
            // Force connection: Use the LAST point of the track (Latest AIS point in time) as base
            // API returns descending hoursAgo (Oldest -> Newest), so the last point is the latest.
            let baseCoords = null;
            if (trackPoints && trackPoints.length > 0) {
                 const lastPoint = trackPoints[trackPoints.length - 1];
                 baseCoords = { lat: lastPoint[0], lng: lastPoint[1] };
            } else {
                 baseCoords = latestTrajectory || scheduledCoords || ship?.coords || null;
            }
